...
describe('integration: manifest', () => {
  ...
  it('generates correct manifest object', async () => {
    ...
    expect(res.items[4].items).to.have.length(15)
    expect(res.items[4].items[0].path).to.equal('installation.md')
    expect(res.items[4].items[0].title).to.equal('Installation')
    expect(res.items[4].items[0].url).to.equal('/gitdocs/installation/')
