...
export default ({ output, docVersion }) => {
  ...
  return {
    paths: {
      ...
      dist: path.resolve(ROOT, output), // The production output directory.
      devDist: path.resolve(ROOT, output), // The production output directory.
      public: path.resolve(ROOT, 'public'), // The public directory (files copied to dist during build)
    },
    getSiteData: () => ({
      config,
      tree,
    ...
    getRoutes: () => {
      const routes = [
        ...
        ...files
          .filter(
            (value, index, self) =>
              value.src && self.findIndex(d => d.path === value.path) === index
          )
          .map(file => ({
            path: file.path,
            component: path.resolve(__dirname, './src/containers/Docs'),
            getData: () => ({
              doc: file,
            }),
          })),
        {
          is404: true,
          component: path.resolve(__dirname, './src/containers/404'),
          getData: () => ({
            doc: {
              path: '',
            },
    ...
    onStart: () => {
      fs.copySync(path.resolve(DOCS_SRC, 'public'), path.resolve(ROOT, output))
    },
    onBuild: () => {
      fs.copySync(path.resolve(DOCS_SRC, 'public'), path.resolve(ROOT, output))
    },
        ...
        return (
          <Html>
            <Head>
              ...
              {config.css && <link rel="stylesheet" href="/custom.css" />}
              <title>{config.title}</title>
            </Head>
            <Body>{children}</Body>
            {config.js && <script src="/custom.js" />}
          </Html>
        )
      }
    },
